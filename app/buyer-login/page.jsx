...
export default function Login() {
  ...
  useEffect(() => {
    if (isLoggedIn && user) {
      router.replace("/"); // Always redirect to buyer homepage
    }
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (userSnap.exists()) {
        ...
        dispatch(setUser({ ...updatedUser, activeRole: "buyer" }));
        console.log("📌 Logged in existing user (as buyer):", updatedUser);
      } else {
        const newUser = {
        ...
        dispatch(setUser({ ...newUser, activeRole: "buyer" }));
        console.log("📌 Registered new user (as buyer):", newUser);
      }
